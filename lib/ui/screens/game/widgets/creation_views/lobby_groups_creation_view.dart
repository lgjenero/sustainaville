...
import 'package:sustainaville/components/lobby_group_character_component.dart';
...
class LobbyGroupsCreationView extends StatefulWidget {
  const LobbyGroupsCreationView({super.key});

  @override
  State<LobbyGroupsCreationView> createState() => _LobbyGroupsCreationViewState();
}

class _LobbyGroupsCreationViewState extends State<LobbyGroupsCreationView> {
                ...
                child: Stack(
                  children: _characters.map((e) {
                    return Positioned(
                      ...
                      child: LobbyGroupsCharacterComponent(
                        key: _charactersKeys[e.id],
                        character: e,
                        play: true,
                        animation: CharacterAnimationEnum.idleDown,
                        thinkingType: ThinkingType.dollars,
                      ),
                    );
                  }).toList(),
                ),
  ...
  void _addCharacter() {
    ...
    setState(() {
      ...
      Offset pos = Offset(_random.nextInt(10) * 48, _random.nextInt(7) * 48);
      int retries = 0;
      while (_charactersPosition.values.any((e) => (e.dx - pos.dx).abs() <= 48 && (e.dy - pos.dy).abs() <= 96)) {
        retries++;
        if (retries > 10) break;
        pos = Offset(_random.nextInt(10) * 48, _random.nextInt(8) * 48);
      }

      _charactersPosition[character.id] = pos;
