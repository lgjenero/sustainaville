...
class OnboardingTooltipsData {
  ...
  List<OnboardingTooltip> get shownOrNextInLine {
    return [
      ...shownTooltips,
      if (nextInLine != null) nextInLine!,
    ];
  }

  OnboardingTooltip? get nextInLine {
    for (var tooltip in OnboardingTooltip.values) {
      if (!shownTooltips.contains(tooltip)) {
        return tooltip;
      }
    }
    return null;
